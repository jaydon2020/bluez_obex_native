...
template <typename T, typename MemberPtr> struct FieldDescriptor {
  const char *name;
  MemberPtr ptr;
};
...
inline size_t read_bytes(const uint8_t *buf, size_t offset, void *out,
                         size_t n) {
...
inline size_t decode_field(const uint8_t *buf, size_t offset,
...
template <typename T, typename Tuple, std::size_t... I>
size_t decode_impl(const uint8_t *buf, size_t offset, T &obj,
...
template <typename T> size_t decode(const uint8_t *buf, size_t offset, T &obj) {
