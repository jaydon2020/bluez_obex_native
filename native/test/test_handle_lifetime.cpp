...
void expect_stale_handle_rejected(void *handle) {
  ...
  assert(bluez_obex_get_managed_objects(handle, nullptr, 0) == -1);
  assert(bluez_obex_client_create_session(handle, value, value, nullptr, 0) ==
         -1);
  assert(bluez_obex_client_remove_session(handle, path) == -1);
  assert(bluez_obex_session_get_properties(handle, path, nullptr, 0) == -1);
  assert(bluez_obex_session_get_capabilities(handle, path, nullptr, 0) == -1);
  assert(bluez_obex_transfer_get_properties(handle, path, nullptr, 0) == -1);
  ...
  assert(bluez_obex_phonebook_get_properties(handle, path, nullptr, 0) == -1);
  ...
  assert(bluez_obex_phonebook_pull(handle, path, value, value, nullptr, nullptr,
                                   0, nullptr, 0) == -1);
  assert(bluez_obex_phonebook_list(handle, path, nullptr, nullptr, 0, nullptr,
                                   0) == -1);
  ...
  assert(bluez_obex_phonebook_list_filter_fields(handle, path, nullptr, 0) ==
         -1);
  assert(bluez_obex_message_access_set_folder(handle, path, value) == -1);
  assert(bluez_obex_message_access_get_properties(handle, path, nullptr, 0) ==
         -1);
  assert(bluez_obex_message_access_list_folders(handle, path, nullptr, nullptr,
                                                0, nullptr, 0) == -1);
  assert(bluez_obex_message_access_list_filter_fields(handle, path, nullptr,
                                                      0) == -1);
  ...
  assert(bluez_obex_message_access_push_message(handle, path, value, value,
                                                ...
                                                0) == -1);
  assert(bluez_obex_message_get_properties(handle, path, nullptr, 0) == -1);
  assert(bluez_obex_message_get(handle, path, value, 1, nullptr, 0) == -1);
